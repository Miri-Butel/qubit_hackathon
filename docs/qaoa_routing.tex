...
target $t_k$, and carries a service-class priority $\pi_k > 0$ ($\pi_k = 1$ by
default). The goal is to select routes that use resources efficiently
...
The raw cost of routing demand $k$ on path $p$ is priority- and
bandwidth-weighted by default,
\begin{equation}
  L_{k,p} \;=\; \pi_k\, b_k \sum_{e \in p} \ell_e
  \qquad \text{(optionally unweighted: } L_{k,p} = \pi_k \textstyle\sum_{e\in p}\ell_e\text{)},
\end{equation}
so that a 10\,Gbps demand on a slow path costs ten times a 1\,Gbps demand, and
a demand with $\pi_k > 1$ pulls proportionally harder toward its low-latency
candidates when several demands contend for the same link.
...
Because the priorities enter $L_{k,p}$ before the extremes are taken, the
normalization is priority-aware: the feasible-space latency spread is exactly
$c_{\mathrm{scale}}$ for any $\{\pi_k\}$, and a global rescale
$\pi_k \mapsto c\,\pi_k$ cancels in \eqref{eq:latnorm} --- only the ratios
between demands are meaningful. Raising a single $\pi_k$ therefore reallocates
a fixed latency budget toward demand $k$ rather than inflating
$\widetilde{H}_{\lat}$ against $\lambda_{\cng}$; the other demands' latency
preferences weaken relative to congestion, which is the intended service-class
semantics.
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
Priorities are deliberately absent from $u_e$: link utilization is a physical
quantity, and the Fortz--Thorup calibration below is fitted against real
loads, so weighting them by service class would distort both the congestion
penalty and the reported KPIs.
...
$\pi_k$ (per demand)       & $1.0$ & service-class priority: latency multiplier \\
\bottomrule
\end{tabular}
\caption{Default Hamiltonian weights. All but $\pi_k$ are global knobs on
\texttt{QuboWeights}; the priority $\pi_k$ is per-demand data carried on
\texttt{Demand}.}
\label{tab:weights}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Traffic demands ($d_1$ is a premium class).}
\label{tab:demands}
\end{minipage}
\end{table}
...
\toprule
Qubit & Demand & $p$ & Path & $\sum_{e\in p}\ell_e$ & $L_{k,p} = \pi_k b_k \ell(p)$ \\
\midrule
$x_0$ & $d_1$ & 0 & A--B--D--F     & 4 & 48 \\
$x_1$ & $d_1$ & 1 & A--B--D--E--F  & 4 & 48 \\
$x_2$ & $d_1$ & 2 & A--C--D--F     & 6 & 72 \\
...
The normalization constants evaluate to $L^{\min} = 66$, $L^{\max} = 102$,
hence $s_{\lat} = 1/36 \approx 0.0278$; all $m_{\used} = 9$ links are crossed by some
...
\texttt{quadratic} (default)
  & (A--B--D--E--F,\; A--C--E,\; B--D--F) & 1.40 & 1 & 40 & 326.3 \\
\texttt{fortz-thorup-fit}
  & (A--B--D--E--F,\; A--C--E,\; B--C--D--F) & \textbf{0.80} & \textbf{0} & 46 & \textbf{2.07} \\
\bottomrule
\end{tabular}
...
optimum --- the congestion term demonstrably reshapes the routing --- but at
...
$\Phi^{*} = 2.07$ (well below the congestion threshold $32/3$), bought with
$+35\%$ bandwidth-weighted latency ($34 \to 46$). This is exactly the
congestion-relief-versus-latency trade-off of the challenge KPIs, exposed
through the $\lambda_{\cng}$ / profile knobs.

\paragraph{Effect of the priority.}
The contention that $\pi_k$ resolves is between $d_1$ and $d_3$: both want
B$\to$D, whose capacity~5 cannot carry $b_1 + b_3 = 7$, so exactly one of them
takes a low-latency route and the other detours. At neutral priorities
($\pi_k \equiv 1$) the feasible optimum routes $d_3$ over B--D--E--F and pushes
the premium demand onto A--C--D--F ($\ell = 6$), for a bandwidth-weighted
latency of $48$ and $\Phi^{*} = 2.08$. Raising $\pi_1$ to $2$ flips the
decision, and it stays flipped for every larger value tested (up to $\pi_1 =
8$), so the demo does not sit on a knife edge: $d_1$ takes A--B--D--E--F
($\ell = 4$) while $d_3$ detours over B--C--D--F ($\ell = 5$). Both routings
are capacity-clean, so this is a genuine preference change rather than a
feasibility artifact --- $\Phi^{*}$ moves only from $2.08$ to $2.07$ and the
maximum utilization is $0.80$ either way, while the premium demand's own
latency drops by a third.
...
against congestion relief (Table~\ref{tab:results}); the per-demand priorities
$\pi_k$ arbitrate between demands competing for the same scarce link, without
touching the physical congestion model; $\lambda_{\switch}$ with
